...
const kickoffRoot =
  process.env.NODICS_KICKOFF_ROOT ||
  path.resolve(import.meta.dirname, "../../../nodics.kickoff");
...
describe("Kickoff logical renderer mappings", () => {
  it("maps Telco content keys to executable code in the Telco frontend composition", () => {
    ...
    const sourceRoot =
      manifest.sections[`agora${title}ContentCatalog`].sourceRoot;
    const data = require(
      path.join(
        kickoffRoot,
        "modules",
        ...
        "content",
        "records",
        `agora${title}RendererData.js`,
      ),
    );
    for (const mapping of Object.values(data) as { renderer: string }[]) {
      const registry = mapping.renderer.includes(".page.")
        ? storefrontPageRendererRegistry
