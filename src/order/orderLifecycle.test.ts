...
describe("order lifecycle contract", () => {
  it("covers cancellation return refund exchange replacement and appeal customer intents", () => {
    ...
    expect(lifecycleFormGuidance.APPEAL).toContain(
      "Reference a rejected or delayed lifecycle case",
    );
  });
  ...
  it("previews lifecycle eligibility without creating the request", async () => {
    ...
    expect(
      lifecyclePayload("order-1", "APPEAL", { appealReferenceCode: "case-1" })
        .evidence?.appealReferenceCode,
    ).toBe("case-1");
    vi.unstubAllGlobals();
  });
  ...
  it("summarizes lifecycle preview and creation evidence for customer order status", () => {
    expect(
      ...
      "SUBMITTED · RMA order-1:RMA:1 · refund REQUIRES_BACKOFFICE_CALCULATION",
    );
    expect(
      lifecycleSummary({
        orderCode: "order-1",
    ...
    expect(
      ...
      "RMA order-1:RMA:1",
      "Inspection required",
      "Refund REQUIRES_BACKOFFICE_CALCULATION",
    ]);
    expect(
      lifecycleTrackingSummary({
        orderCode: "order-1",
        ...
        automationPlan: [
          {
            ...
            owner: "inventory",
            trigger: "agoraCashmereCardigan",
            customerVisibleState: "Replacement selection received",
          },
        ],
      }),
    ).toContain(
      "inventory · replacement-reservation · Replacement selection received · trigger agoraCashmereCardigan",
    );
    expect(
      lifecycleAutomationPlan({
        orderCode: "order-1",
