...
function componentMedia(
  value: unknown,
): readonly CmsComponentMediaContract[] | undefined {
  if (!Array.isArray(value)) return undefined;
  return Object.freeze(
    value.filter(isRecord).map((item) => {
      const media = isRecord(item.media)
        ? Object.freeze({
            ...(typeof item.media.code === "string"
              ? { code: item.media.code }
              : {}),
            ...(typeof item.media.mediaCode === "string"
              ? { mediaCode: item.media.mediaCode }
              : {}),
            ...(typeof item.media.name === "string"
              ? { name: item.media.name }
              : {}),
            ...(typeof item.media.description === "string"
              ? { description: item.media.description }
              : {}),
            ...(typeof item.media.folderCode === "string"
              ? { folderCode: item.media.folderCode }
              : {}),
            ...(typeof item.media.formatCode === "string"
              ? { formatCode: item.media.formatCode }
              : {}),
            ...(typeof item.media.mimeType === "string"
              ? { mimeType: item.media.mimeType }
              : {}),
            ...(optionalNumber(item.media.sizeBytes) !== undefined
              ? { sizeBytes: optionalNumber(item.media.sizeBytes) }
              : {}),
            ...(typeof item.media.extension === "string"
              ? { extension: item.media.extension }
              : {}),
            ...(typeof item.media.access === "string"
              ? { access: item.media.access }
              : {}),
            ...(typeof item.media.businessPurpose === "string"
              ? { businessPurpose: item.media.businessPurpose }
              : {}),
            ...(typeof item.media.ownerType === "string"
              ? { ownerType: item.media.ownerType }
              : {}),
            ...(typeof item.media.ownerReference === "string"
              ? { ownerReference: item.media.ownerReference }
              : {}),
            ...(typeof item.media.reusable === "boolean"
              ? { reusable: item.media.reusable }
              : {}),
            ...(typeof item.media.status === "string"
              ? { status: item.media.status }
              : {}),
            ...(typeof item.media.deliveryUrl === "string"
              ? { deliveryUrl: item.media.deliveryUrl }
              : {}),
            ...(typeof item.media.publicUrl === "string"
              ? { publicUrl: item.media.publicUrl }
              : {}),
          })
        : undefined;
      return Object.freeze({
        ...(typeof item.componentMediaCode === "string"
          ? { componentMediaCode: item.componentMediaCode }
          : {}),
        ...(typeof item.mediaCode === "string"
          ? { mediaCode: item.mediaCode }
          : {}),
        ...(typeof item.mediaSetCode === "string"
          ? { mediaSetCode: item.mediaSetCode }
          : {}),
        ...
        ...(typeof item.localeCode === "string"
          ...
          : {}),
        ...(optionalNumber(item.position) !== undefined
        ...
        ...(typeof item.deliveryUrl === "string"
...
function components(
  ...
  if (depth > 10)
    throw new Error("CMS component graph exceeds the Agora depth limit");
  return Object.freeze(
    value.map((item, index) => {
      budget.count += 1;
      if (budget.count > 200)
        throw new Error("CMS component graph exceeds the Agora size limit");
...
export function parseCmsResolvedPage(value: unknown): CmsResolvedPageContract {
  ...
  const templateContract = isRecord(page.templateContract)
    ? Object.freeze({
