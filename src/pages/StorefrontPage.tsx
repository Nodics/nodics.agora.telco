...
const storefrontProfile = (() => {
  ...
  if (siteCode.includes("electronics")) {
    return {
      ...
      navItems: [
        ...
        {
          label: "Computing",
          collectionCode: "agoraElectronicsComputing",
          dropdown: true,
        },
  ...
  if (siteCode.includes("telco")) {
    return {
      ...
      navItems: [
        { label: "Shop", collectionCode: "", dropdown: true },
        { label: "New in", collectionCode: "agoraNewArrivals" },
        {
          label: "Postpaid",
...
const routeStateFromLocation = function (): RouteState {
  if (typeof window === "undefined")
    return {
      view: "home",
      collectionCode: storefrontProfile.rootCollectionCode,
      query: "",
    };
  ...
  if (path === "/shop" || path === "/apparel" || path === "/clothing")
    return { view: "plp", ...readListingLocation(window.location.search) };
  if (path === "/electronics")
    return {
      view: "plp",
      collectionCode: "agoraElectronicsComputing",
  ...
  if (path === "/telco")
    return { view: "plp", collectionCode: "agoraTelcoPostpaid", query: "" };
  if (path === "/new-in")
    return { view: "plp", collectionCode: "agoraNewArrivals", query: "" };
  if (path === "/bags" || path === "/accessories")
    return { view: "plp", collectionCode: "agoraWomenAccessories", query: "" };
...
const productOptionColourCode = function (option: {
  readonly colourCode?: string;
  readonly colorCode?: string;
}): string | undefined {
  return option.colourCode ?? option.colorCode;
};
const productColorOptions = function (
  product: ProductCard | ProductDetail | undefined,
) {
  const options = new Map<
    string,
    { readonly code: string; readonly label: string; readonly value: string }
  >();
  (product?.apparel?.options ?? []).forEach((option) => {
    ...
    options.set(colourCode, {
      code: colourCode,
      label: displayLabel(colourCode),
      value: swatchPalette[colourCode] ?? "#f6c100",
    });
  });
  return Array.from(options.values());
};
const productSizeOptions = function (
  product: ProductCard | ProductDetail | undefined,
  colourCode?: string,
) {
...
const productVariantForSelection = function (
  product: ProductCard | ProductDetail | undefined,
  colourCode?: string,
  sizeCode?: string,
) {
  return (
    (product?.apparel?.options ?? []).find((option) => {
      if (colourCode && productOptionColourCode(option) !== colourCode)
        return false;
      if (sizeCode && option.sizeCode !== sizeCode) return false;
      return Boolean(option.variantCode);
    })?.variantCode ??
    product?.defaultVariantCode ??
    product?.variantCodes?.[0]
  );
};
...
export function StorefrontPage() {
  ...
  const [brand, setBrand] = useState(initialListingLocation.brand);
  ...
  const quickViewDialog = useRef<HTMLDialogElement>(null);
  const quickViewOpen = Boolean(quickView);
  useEffect(() => {
    if (!quickViewOpen || !quickViewDialog.current) return;
    const dialog = quickViewDialog.current;
  ...
  const [backendListEntryCodes, setBackendListEntryCodes] = useState<
    Readonly<Record<CustomerListType, Readonly<Record<string, string>>>>
  >({ WISHLIST: {}, COMPARE: {} });
  ...
  const activeHeroSlide =
    cmsHeroSlides[activeHeroIndex] ?? cmsHeroSlides[0] ?? emptyHeroSlide;
  const nextHeroSlide =
  ...
  useEffect(() => {
    ...
    void listProducts(runtimeConfig, {
      categoryCode:
        (view === "home" || view === "plp") && collectionCode
          ? collectionCode
          : undefined,
      q: query || undefined,
      ...
      .then((response) => {
        if (active) {
          ...
          setProductTotal(response.total ?? response.products.length);
        }
  ...
  }, [collectionCode, query, view, listingPage, sortCode, brand]);
  ...
  const mergeCustomerList = (
    listType: CustomerListType,
    entries: readonly CustomerListEntry[],
  ) => {
  ...
  const placeOrder = async () => {
    ...
    try {
      ...
      if (customerSession.accessToken && cart.entries.length) {
        try {
          const promotion = await applyPromotion(
            runtimeConfig,
            customerSession.accessToken,
            {
          ...
          if (Number.isFinite(amount) && amount > 0)
            setBackendPromotionDiscount(amount);
          setPromotionStatus(
            promotion.redemption?.code
              ? `Promotion applied ${promotion.redemption.code}`
              : "Promotion eligibility checked",
          );
        } catch {
          setPromotionStatus(
            "Promotion apply unavailable; order uses current visible estimate",
          );
        }
      }
  ...
  const brands = (facets.brands ?? []).flatMap((item) =>
    item &&
    typeof item === "object" &&
    "code" in item &&
  ...
  useEffect(() => {
    if (view !== "plp") return;
    const nextUrl = `/shop${listingLocationSearch(query, collectionCode, brand, sortCode, listingPage)}`;
    const currentUrl = window.location.pathname + window.location.search;
    if (currentUrl === nextUrl) return;
    if (window.location.pathname === "/")
      window.history.pushState({}, "", nextUrl);
    else window.history.replaceState(window.history.state, "", nextUrl);
  }, [view, query, collectionCode, brand, sortCode, listingPage]);
  ...
  const selectedColourCode =
    productOptionColourCode(
      selected?.apparel?.options?.find(
        (option) => option.variantCode === selectedVariantCode,
      ) ?? {},
    ) ?? productOptionColourCode(selected?.apparel?.options?.[0] ?? {});
  const selectedSizeCode = selected?.apparel?.options?.find(
    (option) => option.variantCode === selectedVariantCode,
  )?.sizeCode;
    ...
    <main className="agora-shell">
      ...
      >
        <div className="utility-links">
          ...
          <button
            onClick={() => openCollection(storefrontProfile.rootCollectionCode)}
            type="button"
          >
            Our Store
          </button>
        ...
        >
          <button type="button">
            <span aria-hidden="true"></span> {runtimeConfig.currency}{" "}
            <ChevronDown aria-hidden="true" size={16} />
          </button>
          <button type="button">
            English <ChevronDown aria-hidden="true" size={16} />
          </button>
        </div>
      </aside>
      <header
        ...
        <nav className="nav-pills" aria-label="Storefront navigation">
          ...
          {storefrontProfile.navItems.map((item) => (
            <button
              ...
              key={item.label}
              onClick={() => openCollection(item.collectionCode)}
              type="button"
            >
              {item.label}{" "}
              {item.dropdown ? (
      ...
      {view === "home" ? (
        <>
          <header
            className={`hero hero-fashion ${storefrontProfile.domainClassName}${useCmsHeroImages ? "" : " hero-domain"}`}
          >
            <div
      ...
      {view === "pdp" && selected ? (
        <section className="pdp">
          ...
          <div className="pdp-layout">
            ...
            <article>
              <p className="muted">{selected.brand ?? "Nodics"}</p>
              <h2>{selected.name}</h2>
              ...
              <div className="pdp-tabs">
                {selected.electronics?.specifications &&
                Object.keys(selected.electronics.specifications).length ? (
                  <details open>
                    <summary>Specifications</summary>
                    <dl className="product-specifications">
                      {Object.entries(selected.electronics.specifications)
                        .filter(
                          ([, value]) =>
                            typeof value === "string" ||
                            typeof value === "number",
                ...
                {selected.telco?.allowances?.length ? (
                  <details open>
                    <summary>Plan benefits</summary>
                    <dl className="product-specifications">
                      {selected.telco.allowances.map((allowance, index) => (
                        <div key={`${allowance.type}-${index}`}>
                          <dt>{allowance.type?.replace(/[_-]/g, " ")}</dt>
                          <dd>
                ...
                <details>
                  ...
                  <p>
                    Free shipping threshold and 14-day returns mirror the
                    reference storefront policy.
                  </p>
                </details>
                <details>
              ...
              >
                {homeContent.collections.map((collection) => (
                  <button
                    ...
                    onClick={() => openCollection(collection.code)}
                    type="button"
                  >
        ...
        <>
          ...
          <section className="plp-toolbar">
            <div>
              ...
              <h2>
                {collections.find(
                  (collection) => collection.code === collectionCode,
                )?.label ?? "Collection"}
              </h2>
            </div>
            <div className="chip-row" aria-label="Collection filters">
              {collections.map((collection) => (
                <button
                  className={
                    collection.code === collectionCode ? "" : "secondary"
                  }
                  key={collection.code}
                  onClick={() => {
            ...
            <div className="chip-row" aria-label="Brand filters">
              ...
              {brands.map((nextBrand) => (
                <button
                  className={brand === nextBrand ? "" : "secondary"}
                  key={nextBrand}
                  onClick={() => {
                    setBrand(nextBrand);
                    setListingPage(1);
          ...
          {!visibleProducts.length ? (
            <section className="cart-summary" aria-label="No products found">
              ...
              <p>Try another search or collection to explore more products.</p>
              <button
                onClick={() => {
                  setQuery("");
                  setBrand("");
            ...
            <nav className="load-more" aria-label="Product pages">
              <button
                className="secondary"
                disabled={listingPage <= 1}
                onClick={() => setListingPage((page) => page - 1)}
                type="button"
              >
                Previous
              </button>
              <span>
                Page {listingPage} of{" "}
                {Math.max(1, Math.ceil(productTotal / 12))} · {productTotal}{" "}
                products
              </span>
              <button
                className="secondary"
                disabled={listingPage * 12 >= productTotal}
                onClick={() => setListingPage((page) => page + 1)}
                type="button"
              >
                Next
      ...
      {quickView ? (
        <dialog
          ...
          onCancel={() => setQuickView(undefined)}
        >
          <article>
            <button
            ...
            {productImageUrl(quickView, runtimeConfig.mediaBaseUrl) ? (
              <img
                className="quick-view-product-image"
                alt={quickView.name ?? "Product"}
                src={productImageUrl(quickView, runtimeConfig.mediaBaseUrl)}
              />
            ) : null}
            <h2>{quickView.name}</h2>
            <p>{quickView.summary}</p>
            <p className="price">
              {quickView.price?.currency} {quickView.price?.unitAmount}
            </p>
            <div className="quick-view-actions">
              <button onClick={() => addToCart(quickView)} type="button">
                Quick Add
              </button>
              <button
                className="secondary"
              ...
              <button
                className="secondary"
                onClick={() => openProduct(quickView.productCode)}
                type="button"
              >
                View full details
              </button>
