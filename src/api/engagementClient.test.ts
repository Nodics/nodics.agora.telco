...
describe("engagementClient", () => {
  ...
  it("reads public product review aggregates from Engagement API", async () => {
    vi.mocked(fetch).mockResolvedValueOnce(
      jsonResponse({
        data: { targetCode: "agoraLinenWrapDress", count: 4, average: 4.5 },
      }),
    );
    ...
    expect(lastRequest().target).toContain(
      "http://localhost:4340/nodics/engagementApi/v0/public/review-aggregates/PRODUCT/agoraLinenWrapDress",
    );
  ...
  it("lists sanitized public review projections for PDP display", async () => {
    vi.mocked(fetch).mockResolvedValueOnce(
      jsonResponse({
        data: {
          items: [{ reviewCode: "review-1", overallRating: 5 }],
          total: 1,
        },
      }),
    );
